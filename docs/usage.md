# nf-core/hicscaff: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/hicscaff/usage](https://nf-co.re/hicscaff/usage)
...
```bash
nextflow run nf-core/hicscaff --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```bash
nextflow pull nf-core/hicscaff
```
...
First, go to the [nf-core/hicscaff releases page](https://github.com/nf-core/hicscaff/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/hicscaff`](https://hub.docker.com/r/nfcore/hicscaff/)
* `singularity`
